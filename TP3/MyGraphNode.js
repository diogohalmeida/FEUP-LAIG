    ...
    display(fatherMaterialID, fatherTextureID){
        ...
        for (var i = 0; i < this.children.length; i++) {
            ...
            if (this.graph.nodes[this.children[i]] != null) {
                if (this.graph.nodes[this.children[i]].animation != null) {
                    if(this.graph.nodes[this.children[i]].animation.startedMoving){
                        this.graph.nodes[this.children[i]].display(materialID, textureID);
                    }
                }
                else{
                    if ("victoryConfetti1" == this.children[i] || "victoryConfetti2" == this.children[i]){
                        if (this.graph.scene.gameOrchestrator.state == 2)
                            this.graph.nodes[this.children[i]].display(materialID, textureID);
                    }
                    else{
                        this.graph.nodes[this.children[i]].display(materialID, textureID);
