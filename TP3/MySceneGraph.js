...
class MySceneGraph {
    ...
    parseSpriteSheets(spriteSheetsNode){
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if(this.spriteSheets[spriteSheetID] != null)
                return "ID must be unique for each sprite sheet (conflict: ID = " + spriteSheetID + ")";
  ...
  parseNodes(nodesNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            for (var l = 0; l < transformationsChildren.length; l++) {
                switch(transformationsChildren[l].nodeName){
                    case 'translation':
                        var translation = this.parseCoordinates3D(transformationsChildren[l],'Translation');
                        if (!Array.isArray(translation))
                            this.onXMLError(translation);
                        mat4.translate(this.nodes[nodeID].transformationMatrix,this.nodes[nodeID].transformationMatrix,translation);
                        break;
                    case 'rotation':
            ...
            if (animationIndex != -1){
                ...
                if (animationID == null){
                    return "<animationref> needs a valid id at node with id " + nodeID;
                }
            ...
            for (var m = 0; m < descendantsChildren.length; m++) {
                if(descendantsChildren[m].nodeName == 'noderef'){
                    ...
                    if(type == 'rectangle'){
                        ...
                        this.nodes[nodeID].addLeaf(this.ProcessSpriteText(descendantsChildren[m]));
                    }
                    else if(type == 'spriteanim'){
                        this.nodes[nodeID].addLeaf(this.ProcessSpriteAnimation(descendantsChildren[m]));
                    }
                    else if(type == 'plane'){
                        this.nodes[nodeID].addLeaf(this.ProcessPlane(descendantsChildren[m]));
                    }
                    else if(type == 'patch'){
                        this.nodes[nodeID].addLeaf(this.ProcessPatch(descendantsChildren[m]));
                    }
                    else if(type == 'defbarrel'){
                        this.nodes[nodeID].addLeaf(this.ProcessDefBarrel(descendantsChildren[m]));
                    }
    ...
    ProcessDefBarrel(node){
        var base = this.reader.getFloat(node,'base');
        var middle = this.reader.getFloat(node,'middle');
        ...
        if(base == null || isNaN(base) || base <= 0)
            this.onXMLMinorError("unable to parse base of the def barrel. It must be a number greater than zero");

        if(middle == null || isNaN(middle) || middle <= 0)
            this.onXMLMinorError("unable to parse middle of the def barrel. It must be a number greater than zero");  

        if(height == null || isNaN(height) || height <= 0)
            this.onXMLMinorError("unable to parse height of the def barrel. It must be a number greater than zero"); 

        if(slices == null || isNaN(slices) || slices <= 0)
            this.onXMLMinorError("unable to parse slices of the def barrel. It must be a integer greater than zero"); 

        if(stacks == null || isNaN(stacks) || stacks <= 0)
            this.onXMLMinorError("unable to parse stacks of the def barrel. It must be a integer greater than zero");

        return new DefBarrel(this.scene,base,middle,height,slices,stacks);
    ...
    ProcessPatch(node){
        ...
        if(nPartsU == null || isNaN(nPartsU) || nPartsU <= 0)
            this.onXMLMinorError("unable to parse nPartsU of the patch. It must be a integer greater than zero");

        if(nPartsV == null || isNaN(nPartsV) || nPartsV <= 0)
            this.onXMLMinorError("unable to parse nPartsV of the patch. It must be a integer greater than zero");  

        if(nPointsU == null || isNaN(nPointsU) || nPointsU <= 0)
            this.onXMLMinorError("unable to parse nPointsU of the patch. It must be a integer greater than zero"); 

        if(nPointsV == null || isNaN(nPointsV) || nPointsV <= 0)
            this.onXMLMinorError("unable to parse nPointsV of the patch. It must be a integer greater than zero");


        for (var i = 0; i < node.children.length; i++) {
            var x = this.reader.getFloat(node.children[i],'x');
            var y = this.reader.getFloat(node.children[i],'y');
            var z = this.reader.getFloat(node.children[i],'z');   
    ...
    ProcessPlane(node){
        ...
        if(nPartsU == null || isNaN(nPartsU) || nPartsU <= 0)
            this.onXMLMinorError("unable to parse nPartsU of the plane. It must be a integer greater than zero");

        if(nPartsV == null || isNaN(nPartsV) || nPartsV <= 0)
            this.onXMLMinorError("unable to parse nPartsV of the plane. It must be a integer greater than zero");  


        return new Plane(this.scene,nPartsU,nPartsV);
    }

    ProcessSpriteAnimation(node){
        var ssid = this.reader.getString(node,'ssid');
        var startCell = this.reader.getInteger(node,'startCell');
        var endCell = this.reader.getInteger(node,'endCell');
        var duration = this.reader.getFloat(node,'duration');

        if(ssid == null)
            this.onXMLMinorError("unable to parse base of the ssid of the sprite animation");

        if(startCell == null || isNaN(startCell) || startCell < 0)
            this.onXMLMinorError("unable to parse startCell of the sprite animation. It must be a integer greater or equal than zero");  

        if(endCell == null || isNaN(endCell) || endCell < 0)
            this.onXMLMinorError("unable to parse endCell of the sprite animation. It must be a integer greater than zero"); 

        if(duration == null || isNaN(duration) || duration <= 0)
            this.onXMLMinorError("unable to parse duration of the sprite animation. It must be a number greater than zero");
    ...
    ProcessSpriteText(node){
        var text = this.reader.getString(node,'text');

        if(text == null)
            this.onXMLMinorError("unable to parse text of the sprite text");
    ...
    ProcessRectanglePrimitive(node){
        ...
        if(x1 == null || isNaN(x1))
            this.onXMLMinorError("unable to parse x1-coordinate of the rectangle");

        var x2 = this.reader.getFloat(node,'x2');

        if(x2 == null || isNaN(x2))
            this.onXMLMinorError("unable to parse x2-coordinate of the rectangle");

        var y1 = this.reader.getFloat(node,'y1');

        if(y1 == null || isNaN(y1))
            this.onXMLMinorError("unable to parse y1-coordinate of the rectangle");

        var y2 = this.reader.getFloat(node,'y2');

        if(y2 == null || isNaN(y2))
            this.onXMLMinorError("unable to parse y2-coordinate of the rectangle");
    ...
    ProcessSpherePrimitive(node){
        ...
        if(slices == null || isNaN(slices) || slices < 1)
            this.onXMLMinorError("unable to parse slices of the sphere. It must be an positive integer greater than 1");
        ...
        if(radius == null || isNaN(radius) || radius <= 0)
            this.onXMLMinorError("unable to parse slices of the sphere. It must be a positive number greater than zero");
    ...
    ProcessCylinderPrimitive(node){
        ...
        if(slices == null || isNaN(slices) || slices < 1)
            this.onXMLMinorError("unable to parse slices of the cylinder. It must be an positive integer greater than 1");
        ...
        if(topRadius == null || isNaN(topRadius) || topRadius < 0)
            this.onXMLMinorError("unable to parse topRadius of the cylinder. It must be a positive number greater than zero");

        var bottomRadius = this.reader.getFloat(node,'bottomRadius');

        if(bottomRadius == null || isNaN(bottomRadius) || bottomRadius < 0)
            this.onXMLMinorError("unable to parse bottomRadius of the cylinder. It must be a positive number greater than zero");

        var height = this.reader.getFloat(node,'height')

        if(height == null || isNaN(height) || height <= 0)
            this.onXMLMinorError("unable to parse height of the cylinder. It must be a positive number greater than zero");
    ...
    ProcessTrianglePrimitive(node){
        ...
        if(x1 == null || isNaN(x1))
            this.onXMLMinorError("unable to parse x1-coordinate of the triangle");

        var y1 = this.reader.getFloat(node,'y1');

        if(y1 == null || isNaN(y1))
            this.onXMLMinorError("unable to parse y1-coordinate of the triangle");

        var x2 = this.reader.getFloat(node,'x2');

        if(x2 == null || isNaN(x2))
            this.onXMLMinorError("unable to parse x2-coordinate of the triangle");

        var y2 = this.reader.getFloat(node,'y2');

        if(y2 == null || isNaN(y2))
            this.onXMLMinorError("unable to parse y2-coordinate of the triangle");

        var x3 = this.reader.getFloat(node,'x3');

        if(x3 == null || isNaN(x3))
            this.onXMLMinorError("unable to parse x3-coordinate of the triangle");

        var y3 = this.reader.getFloat(node,'y3');

        if(y3 == null || isNaN(y3))
            this.onXMLMinorError("unable to parse y3-coordinate of the triangle");
    ...
    ProcessTorusPrimitive(node){
        ...
        if(innerRadius == null || isNaN(innerRadius) || innerRadius <= 0)
            this.onXMLMinorError("unable to parse inner of the torus. It must be a positive number greater than zero");

        var outerRadius = this.reader.getFloat(node,'outer');

        if(outerRadius == null || isNaN(outerRadius) || outerRadius <= 0)
            this.onXMLMinorError("unable to parse outer of the torus. It must be a positive number greater than zero");

        var slices = this.reader.getInteger(node,'slices');

        if(slices == null || isNaN(slices) || slices < 1)
            this.onXMLMinorError("unable to parse slices of the torus. It must be an positive integer greater than 1");

        var loops = this.reader.getInteger(node,'loops');

        if(loops == null || isNaN(loops) || loops < 1)
            this.onXMLMinorError("unable to parse loops of the torus. It must be an positive integer greater than 1");
    ...
    parseAnimations(animationsNode) {
        ...
        for (var animation of animations){
            if (animation.nodeName != "animation"){
                return "unknown tag in animations " ;
            }
            
            var animationID = this.reader.getString(animation, "id");

            var keyframes = animation.children;

            if (keyframes.length == 0 || keyframes == null){
                return "animation has no keyframes" ;
            }

            var newAnimation = new KeyFrameAnimation(this.scene, animationID);
            for (var keyframe of keyframes){
                if (keyframe.nodeName != "keyframe"){
                    return "unknown tag in animation keyframe ";
                }

                var instant = this.reader.getFloat(keyframe, "instant");
                if (instant == null || isNaN(instant))
                    return "unable to parse instant of the animation " ;
                
                //Translation
                var translationX = this.reader.getFloat(keyframe.children[0],'x');
                
                if (translationX == null || isNaN(translationX))
                    return "unable to parse translationX of the animation " ;

                var translationY = this.reader.getFloat(keyframe.children[0],'y');
                if (translationY == null || isNaN(translationY))
                    return "unable to parse translationY of the animation " ;
                
                var translationZ = this.reader.getFloat(keyframe.children[0],'z');
                if (translationZ == null || isNaN(translationZ))
                    return "unable to parse translationZ of the animation " ;

                var translationCoords = [translationX, translationY, translationZ];

                //Rotation
                //X
                var axisX = this.reader.getString(keyframe.children[1],'axis');
                if (axisX != "x")
                    return "unable to parse axisX of the animation ";
                var rotationX = this.reader.getFloat(keyframe.children[1],'angle');
                if (rotationX == null || isNaN(rotationX))
                    return "unable to parse rotationX of the animation " ;

                //Y
                var axisY = this.reader.getString(keyframe.children[2],'axis');
                if (axisY != "y")
                    return "unable to parse axisY of the animation ";   
                var rotationY = this.reader.getFloat(keyframe.children[2],'angle');
                if (rotationY == null || isNaN(rotationY))
                    return "unable to parse rotationY of the animation " ;

                //Z
                var axisZ = this.reader.getString(keyframe.children[3],'axis');
                if (axisZ != "z")
                    return "unable to parse axisZ of the animation "; 
                var rotationZ = this.reader.getFloat(keyframe.children[3],'angle');
                if (rotationZ == null || isNaN(rotationZ))
                    return "unable to parse rotationZ of the animation " ;
                
                var rotationCoords = [rotationX * DEGREE_TO_RAD, rotationY * DEGREE_TO_RAD, rotationZ * DEGREE_TO_RAD];
                
                //Scale
                var scaleX = this.reader.getFloat(keyframe.children[4],'sx');
                if (scaleX == null || isNaN(scaleX))
                    return "unable to parse scaleX of the animation " ;

                var scaleY = this.reader.getFloat(keyframe.children[4],'sy');
                if (scaleY == null || isNaN(scaleY))
                    return "unable to parse scaleY of the animation " ;
                    
                var scaleZ = this.reader.getFloat(keyframe.children[4],'sz');
                if (scaleZ == null || isNaN(scaleZ))
                    return "unable to parse scaleZ of the animation " ;

                var scaleCoords = [scaleX, scaleY, scaleZ];
                
                //New Keyframe Creation
                var newKeyframe = new Keyframe(instant, translationCoords, rotationCoords, scaleCoords);
                newAnimation.addKeyframe(newKeyframe);
            }
